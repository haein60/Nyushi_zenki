...
Private Sub Form_Load()
    ...
    lblMsg.FontSize = 10                              '2021.12.21 add jhi
...
Private Sub cmdOK_Click()
    ...
    If txtCSVPath.Text = "" Then
        lblMsg.Visible = True
        lblMsg.Caption = "ファイルを指定してください。" ''''LoadResString(1730)
    Else
        lblMsg.Visible = False
        lblMsg.Caption = ""
        cmdOK.Enabled = False
...
RetData:
    ...
    If f_bln_ReturnVal = 0 Then
        lblMsg.Visible = True
        lblMsg.Caption = "Web出願データを受験生プロファイルテーブルにImportしました。"    ''''LoadResString(1726)

    ElseIf f_bln_ReturnVal = -1 Then
        lblMsg.Visible = True
        lblMsg.Caption = "定義ファイル(" & iniFile & ")の読み込みに失敗しました。"

    ElseIf f_bln_ReturnVal = -2 Then
        lblMsg.Visible = True
        lblMsg.Caption = "ODBCへの接続に失敗しました。"

    ElseIf f_bln_ReturnVal = -3 Then
        lblMsg.Visible = True
        lblMsg.Caption = "受験者プロファイルテーブルにデータを挿入中エラーが発生しました。もう一度やってください。"    ''''LoadResString(1727)

    ElseIf f_bln_ReturnVal = -4 Then
        lblMsg.Visible = True
        lblMsg.Caption = "入力ファイル(" & sFileName & ")の読み込みに失敗しました。"

    ElseIf f_bln_ReturnVal = -5 Then
        lblMsg.Visible = True
        lblMsg.Caption = "データベースのコマンド呼び出しに失敗しました。"

    ElseIf f_bln_ReturnVal = 1 Then
        lblMsg.Visible = True
        lblMsg.Caption = "指定のパスにファイルがありません。"
    End If
