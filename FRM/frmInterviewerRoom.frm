...
Begin VB.Form frmInterviewerRoom 
   ClientHeight    =   10335
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   13080
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   Picture         =   "frmInterviewerRoom.frx":0000
   ScaleHeight     =   10335
   ScaleWidth      =   13080
   WindowState     =   2  '最大化
   ...
   End
End
Attribute VB_Name = "frmInterviewerRoom"
...
Private f_bln_FormLoaded As Boolean               ' to check whether form is loaded or not

Private Const prvclCellBackColor As Long = &H80000005
Private Const prvclErrCellBackColor As Long = vbYellow

Private Sub l_ClearErr()
...
'試験官のダブりチェックをする
    With vsfInterviewerRoom
        For lRow = 1 To .Rows - 1
            ...
            For lCol = 0 To .cols - 1
                .Col = lCol
                .CellBackColor = prvclCellBackColor
            Next
        Next
        For lRow = 1 To .Rows - 1
            For lCol = prvlRoomStartCol To .cols - 2 Step 2
                If .TextMatrix(lRow, lCol + 1) = "" Then Exit For
                If lCol + 2 < .cols Then
                    For lCol2 = lCol + 2 To .cols - 2 Step 2
                        If .TextMatrix(lRow, lCol2 + 1) = .TextMatrix(lRow, lCol + 1) Then
                            bErr = True
                            .Col = lCol + 1
                            .Row = lRow
                            .CellBackColor = prvclErrCellBackColor
                            .Col = lCol2 + 1
                            .Row = lRow
                            .CellBackColor = prvclErrCellBackColor
                        End If
                    Next
                End If
                For lRow2 = lRow + 1 To .Rows - 1
                    For lCol2 = prvlRoomStartCol To .cols - 2 Step 2
                        If .TextMatrix(lRow2, lCol2 + 1) = .TextMatrix(lRow, lCol + 1) Then
                            bErr = True
                            .Col = lCol + 1
                            .Row = lRow
                            .CellBackColor = prvclErrCellBackColor
    ...
    If bErr Then
        lblErrorDetails.Caption = "採点者が重複しています。"
...
'行ループ：１行は担当者
        For lRow = 1 To .Rows - 1
            '担当者IDを保持
            sRoomID = .TextMatrix(lRow, 0)
'列ループ：複数の部屋および乱数
            For lCol = prvlRoomStartCol To .cols - 2 Step 2
                sInterviewerID = Trim(.TextMatrix(lRow, lCol))
                If sInterviewerID = "" Then Exit For
...
'add,xzg,2009/12/22,E-----------
    ...
    l_str_sqlRoom = "SELECT "
    l_str_sqlRoom = l_str_sqlRoom & "  iInterviewerProfileId "
    l_str_sqlRoom = l_str_sqlRoom & ", vInterviewerName "
    l_str_sqlRoom = l_str_sqlRoom & "  FROM tbSTEInterviewerProfile "
'    l_str_sqlRoom = l_str_sqlRoom & " WHERE siInterviewFlag = 1"
    'add,xzg,2009/12/22,S-----------
    If Len(strTeacher) > 0 Then
    l_str_sqlRoom = l_str_sqlRoom & " WHERE vInterviewerName like '%" & strTeacher & "%'"
    End If
    'add,xzg,2009/12/22,E-----------
...
Dim sWk As String
    ...
    sSQL = "SELECT iRoomProfileid,vRoomName FROM tbSTERoomProfile"
    sSQL = sSQL & " WHERE iInterviewRoomFlag = 0 "
    sSQL = sSQL & "   AND iMaxCapacity > 0 "
    ...
    With vsfInterviewerRoom
        ...
        Do Until oRs.EOF
            ...
            sSQL2 = "SELECT "
            sSQL2 = sSQL2 & "  iv.iInterviewerProfileId "
            sSQL2 = sSQL2 & ", iv.vInterviewerName "
            sSQL2 = sSQL2 & "  FROM tbSTEInterviewRoomProfile as ir "
            sSQL2 = sSQL2 & " INNER JOIN tbSTEInterviewerProfile as iv "
            sSQL2 = sSQL2 & "    ON iv.iInterviewerProfileId = ir.iInterviewerProfileId "
            sSQL2 = sSQL2 & " WHERE ir.iRoomProfileID = " & Trim(str(oRs.Fields(0)))
            sSQL2 = sSQL2 & "   AND ir.iNendo = " & g_int_CurrentNendo
            sSQL2 = sSQL2 & "   AND ir.iSubjectProfileId = " & cboSubject.ItemData(cboSubject.ListIndex)
            sSQL2 = sSQL2 & "   AND ir.iDayFlag = " & Trim(str(Me.cboDay.ListIndex))
            sSQL2 = sSQL2 & "   AND ir.iRandomNo is null "
'            sSQL2 = sSQL2 & "   AND iv.siInterviewFlag = 1"
