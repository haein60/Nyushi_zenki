VERSION 5.00
Begin VB.Form frm2jigoukaku 
   Caption         =   "frm2jigoukaku : 合格者入力(2次試験))"
   ClientHeight    =   9795
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   14190
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   Picture         =   "frm2jigoukaku.frx":0000
   ScaleHeight     =   9795
   ScaleWidth      =   14190
   WindowState     =   2  '最大化
   ...
   End
End
Attribute VB_Name = "frm2jigoukaku"
...
'*************************************************************************************************
'Form Name      : frm2jigoukaku
'Author         : jhi
'Created On     : 2021.12.30
'Description    : 2次試験 - 合格者入力
'Reference      :
'***************************************************************************************************
...
''''Call g_void_SetFontProperties(Me)     ' set the font properties
    ...
    m_int_Action = 3 '2021.12.30 強制的 2次試験- 合格者入力 flagをセット
...
'        lblSourceJuken.Caption = "合格者番号"
'        lblSelectFrom.Caption = "受験生リスト"  ''''LoadResString(2408)
'        lblSelected.Caption = "合格者リスト"    ''''LoadResString(2410)
'        lblTotal.Caption = "合格者数"           ''''LoadResString(2490)
'
'        cmdOK.Caption = "2次 合格者 確定"
    ...
    txtTotal.Text = lstSelected.ListCount    '合格者count
...
'    End If
    ...
    Case 3  ' passed person data for 2nd phase
        sSQL = sSQL & "    AND iAbsentFlag     =0" & vbCrLf
        sSQL = sSQL & "    AND iExamineeStatus =" & gclExamineeStatus_2ndPass
    End Select
...
Private Sub cmdOK_Click()
    ...
    lblErrorDetails.Caption = "合格者の更新処理を行っています。しばらくお持ちください。"
    cmdOK.Enabled = False
    ...
    If lstSelected.ListCount > 0 Or lstExaminees.ListCount > 0 Then
        ...
        lblErrorDetails.Caption = "合格者の更新処理が正常に終了しました。"    ''''LoadResString(2404)
...
''''g_obj_Conn.RollbackTrans
    lblErrorDetails.Caption = "合格者の更新処理でエラーが発生しました。"      ''''LoadResString(2405)
    MsgBox Err.Description, vbInformation, "エラー"                           ''''LoadResString(1729)
...
Private Sub cmdGoukakuList_Click()
    ...
    'FSOオブジェクットを初期化
    strFile = App.Path & "\Report\2次合格者一覧_" & Format(Now(), "YYYYMMDDHHmmSS") & ".csv"
    Set fso = CreateObject("Scripting.FileSystemObject")
    Set objText = fso.CreateTextFile(strFile, True, False)
...
ErrorHandler:
    ...
    If blnOpenFile = True Then
        ...
        Set fso = Nothing
    End If
    MsgBox Err.Description, vbInformation, "2次合格者一覧表"
