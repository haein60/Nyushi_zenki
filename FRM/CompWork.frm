...
Dim intChoScore1            As Double  '一回目調点
Dim intAveScore1            As Double   '一回目平均
Dim intP1Score1             As Double  '一回目採1
Dim intP2Score1             As Double  '一回目採1
Dim intP3Score1             As Double  '一回目採3
Dim intP4Score1             As Double  '一回目採4
Dim intP5Score1             As Double  '一回目採5
Dim intTotalScore2          As Double   '2回目得点
Dim intChoScore2            As Double  '2回目調点
Dim intAveScore2            As Double   '2回目平均
Dim intP1Score2             As Double  '2回目採1
Dim intP2Score2             As Double  '2回目採2
Dim intP3Score2             As Double  '2回目採3
Dim intP4Score2             As Double  '2回目採4
Dim intP5Score2             As Double  '2回目採5
...
Dim iLogFlag As Integer
Dim log   As Object
Dim objTextLog As Object
...
On Error GoTo ERR_HANDLE
    ...
    logFile = App.Path & "\Log\" & "ComWork" & Year(Now) & ".log"
    
    If log.FileExists(logFile) Then
        Set objTextLog = log.OpenTextFile(logFile, ForAppending)
    Else
        Set objTextLog = log.CreateTextFile(logFile, False)
    End If
    iLogFlag = 1
    objTextLog.WriteLine "入試へ転送--------- " & Now
    ...
    If RS.EOF Then
        objTextLog.WriteLine "受験番号と採点者--------- " & Now
        objTextLog.Close
    ...
    Set RS = conn.Execute(SQL)
    If RS.EOF Then
        objTextLog.WriteLine "NO RoomID--------- " & Now
        objTextLog.Close
        Set objTextLog = Nothing
        Set log = Nothing
        Exit Sub
...
'    sql = sql & " AND  c.iRoomProfileId =" & iRoomProfileid
    ...
    Set rst1 = conn.Execute(SQL)
    If rst1.EOF Then
        objTextLog.WriteLine "no iSubjectQuestionId --------- " & Now
        objTextLog.Close
        Set objTextLog = Nothing
        Set log = Nothing
        Exit Sub
...
'    sql = sql & " AND dtSecondExamDay = '" & strTestdate & "'"
    ...
    Set RS = conn.Execute(SQL)
    If RS.EOF Then
        objTextLog.WriteLine "NO iExamineeProfileId 1--------- " & Now
        objTextLog.Close
        Set objTextLog = Nothing
        Set log = Nothing
        Exit Sub
...
'            sql = sql & " AND sp.siSerialNo=1"
    ...
    blnUpdatedFlag = False
         ...
         objTextLog.WriteLine "入試へ登録完了--------- " & Now
        objTextLog.Close
        Set objTextLog = Nothing
        Set log = Nothing
...
'        conn.Close
    ...
    If iLogFlag = 1 Then
             objTextLog.WriteLine "入試へ登録Error " & Err.Description & Now
        objTextLog.Close
        Set objTextLog = Nothing
        Set log = Nothing
    End If
