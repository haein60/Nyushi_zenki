VERSION 5.00
Begin VB.Form frm1jikesseki 
   Caption         =   "frm1jikesseki : 欠席者入力"
   ClientHeight    =   9795
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   12765
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   Picture         =   "frm1jikesseki.frx":0000
   ScaleHeight     =   9795
   ScaleWidth      =   12765
   WindowState     =   2  '最大化
   ...
   Begin VB.ComboBox cboSubject 
      ...
      ForeColor       =   &H00800000&
      Height          =   360
      ItemData        =   "frm1jikesseki.frx":3AD3
      Left            =   1785
      List            =   "frm1jikesseki.frx":3AD5
      Style           =   2  'ﾄﾞﾛｯﾌﾟﾀﾞｳﾝ ﾘｽﾄ
      TabIndex        =   0
      Top             =   1080
      Width           =   2355
   ...
   End
End
Attribute VB_Name = "frm1jikesseki"
...
'*************************************************************************************************
'Form Name      : frm1jikesseki
'Author         : jhi
'Created On     : 2021.12.29
'Description    : 1次試験 - 欠席者入力
'Reference      :
'***************************************************************************************************
...
Private Sub Form_Load()
    ...
    m_int_Action = 0 '2021.12.29 強制的 1次試験- 欠席者入力 flagをセット
...
''''        Me.Caption = "frm1jikesseki : 1次 欠席者入力"      ''''LoadResString(1010)
''''        lblSelectFrom.Caption = "受験生リスト"                  ''''LoadResString(2408)
''''        lblSelected.Caption = "欠席者リスト"                    ''''LoadResString(2409)
''''        lblTotal.Caption = "欠席者数"                           ''''LoadResString(2489)
        ...
        '受験生List,欠席者Listの上にあるLabel,Text
''''        Label1.Visible = False         '受験番号label
''''        txtSourceJuken.Visible = False '受験番号text

        Label4.Visible = False         '受験番号label
        txtDestJuken.Visible = False   '受験番号text


        cmdOK.Caption = "1次 欠席者 確定"
...
'    If f_bln_FormLoaded Then Call f_void_SelectAbsentee
    ...
    '2021.12.29 add jhi
    If m_int_Action = 0 Then '1次試験、欠席者入力ではこの処理をする必要がないから
        ''''Exit Sub
    End If


    
''''L_str_temp = UCase(LoadResString(2474)) & "*"    ''''2472 面接
    L_str_temp = "面接" & "*"                        ''''2472 面接
    ...
    If f_bln_FormLoaded Then
        lblErrorDetails.Caption = ""    ''''2022.01.15 add jhi
        Call f_void_SelectAbsentee
...
Private Sub f_void_cboSubject_Get()
    ...
    If m_int_Action = 0 Then
''''    sSQL = sSQL & "WHERE iExamType =" & g_int_ExamType   ''''2021.12.29 del jhi
        sSQL = sSQL & "WHERE iExamType =1"                   ''''2021.12.29 add jhi 強制的 1次試験を設定

''''    ElseIf m_int_Action = 2 Or m_int_Action = 3 Then     ''''ここでは, 1次試験のみ対応する
...
''''2021.12.28 del jhi なぜこれを追加するのか？
    If m_int_Action = 0 Then
''''    cboSubject.AddItem prvcSubName_Science, 0  '理科 物理、化学、生物を全部欠席者として設定するのだ。2022.01.27 松沢さん
''''    cboSubject.AddItem prvcSubName_Language, 0 '英語

        ''''理科 物理、化学、生物を全部欠席者として設定するのだ。2022.01.27 松沢さん 2:表示位置変更
        cboSubject.AddItem prvcSubName_Science, 2
    End If
...
''''L_str_temp = UCase(LoadResString(2474)) & "*" ''''2021.12.29 del jhi
    L_str_temp = "面接" & "*"                     ''''2021.12.29 add jhi
...
'入試試験実施時不具合No6対応  2005/01/22 数学のとき、欠席にならない不具合修正。
...
LoopEnd2:
        ...
        If f_bln_DataChanged Then
            ...
            cmdOK.Enabled = False
        End If
        lblErrorDetails.Caption = "欠席者の更新処理が正常に終了しました。" ''''LoadResString(2404)
...
ErrorHandler:
    ...
    lblErrorDetails.Caption = "欠席者の更新処理でエラーが発生しました。"   ''''LoadResString(2405)
    MsgBox Err.Description, vbInformation, "エラー"
...
Private Sub txtSourceJuken_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        If l_obj_rsExaminee.EOF Then
            ' the input juken number does not exist - display an error message
            lblErrorDetails.Caption = "指定された受験番号は存在しません。" ''''LoadResString(2473)
        Else
            lblErrorDetails.Caption = ""
            ' pad the input juken number with leading "0"
...
Private Sub cmdJukenList_Click()
    ...
    'FSOオブジェクットを初期化
    strFile = App.Path & "\Report\1次受験生一覧_会場_" & cboRoom.Text & "_" & Format(Now(), "YYYYMMDDHHmmSS") & ".csv"
...
Private Sub cmdKessekiList_Click()
    ...
    'FSOオブジェクットを初期化
    strFile = App.Path & "\Report\1次欠席者一覧_会場_" & cboRoom.Text & "_" & Format(Now(), "YYYYMMDDHHmmSS") & ".csv"
    Set fso = CreateObject("Scripting.FileSystemObject")
    Set objText = fso.CreateTextFile(strFile, True, False)
...
ErrorHandler:
    ...
    If blnOpenFile = True Then
        ...
        Set fso = Nothing
    End If
    MsgBox Err.Description, vbInformation, "1次欠席者一覧表"
