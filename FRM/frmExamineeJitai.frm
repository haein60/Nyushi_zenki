...
Begin VB.Form frmExamineeJitai 
   Caption         =   "frmExamineeaJitai : é´ëﬁ"
   ClientHeight    =   10095
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   15885
   ...
   Begin VB.Label lblGuidance1 
      BackStyle       =   0  'ìßñæ
      Caption         =   """çáäié“ÉäÉXÉg""Ç©ÇÁ ""ç°âÒé´ëﬁé“""Ç…ì¸ÇÍÅA"
      BeginProperty Font 
         Name            =   "ÇlÇr ÉSÉVÉbÉN"
         Size            =   9
         Charset         =   128
...
''''Call g_void_SetFontProperties(Me)     ' set the font properties
    ...
    m_int_Action = 6 '2021.12.29 ã≠êßìI é´ëﬁèàóù flagÇÉZÉbÉg
...
''''Exit Sub ''''2022.01.16 add jhi ----> del ïKóvÇæÇ¡ÇΩ
...
Private Sub cmdOK_Click()
    ...
    ''''2021.12.15 add jhi
    rinf = myMsgBox("é´ëﬁé“ämíËèàóùÇé¿çsÇµÇ‹Ç∑ÅBÇÊÇÎÇµÇ¢Ç≈Ç∑Ç©ÅH", gTit)
...
'                    " AND iExamineeStatus IN(2,6)"
        ...
        Case Else
            MsgBox "é´ëﬁé“ämíËÉtÉâÉOàŸèÌ(m_int_Action=" & m_int_Action & ")"
            GoTo ErrorHandler
        End Select
        ...
        lblMsg.Caption = "é´ëﬁé“ämíËèàóùÇ™ê≥èÌÇ…èIóπÇµÇ‹ÇµÇΩÅB" ''''LoadResString(2404):çXêVèàóùÇÕê≥èÌÇ…äÆóπÇµÇ‹ÇµÇΩÅB
...
ErrorHandler:
    ...
    lblMsg.Caption = "é´ëﬁé“ämíËèàóùÇ≈ÉGÉâÅ[Ç™î≠ê∂ÇµÇ‹ÇµÇΩÅB" ''''LoadResString(2405)"
    MsgBox Err.Description, vbInformation, "ÉGÉâÅ["           ''''LoadResString(1729)
...
'***************************************************************************
'* çáäié“ÉäÉXÉgÇListBoxÇ…ï\é¶Ç∑ÇÈèàóù                                     *
'***************************************************************************
...
''''----------------------------------------------------------------------------
''''txtDestJukenÇçÌèúÇµÇΩÇΩÇﬂ comment out
''''2022.01.04 del jhi
''''----------------------------------------------------------------------------
...
Private Sub cmdGoukakuList_Click()
    ...
    strFile = App.Path & "\Report\çáäié“àÍóó_é´ëﬁ" & Format(Now(), "YYYYMMDDHHmmSS") & ".csv"
...
ErrorHandler:
    ...
    If blnOpenFile = True Then
        ...
        Set fso = Nothing
    End If
    MsgBox Err.Description, vbInformation, "çáäié“àÍóóï\(é´ëﬁ)"
...
Private Sub cmdJitaiList_Click()
    ...
    'FSOÉIÉuÉWÉFÉNÉbÉgÇèâä˙âª
    strFile = App.Path & "\Report\é´ëﬁé“àÍóó" & Format(Now(), "YYYYMMDDHHmmSS") & ".csv"
    Set fso = CreateObject("Scripting.FileSystemObject")
    Set objText = fso.CreateTextFile(strFile, True, False)
...
ErrorHandler:
    ...
    If blnOpenFile = True Then
        ...
        Set fso = Nothing
    End If
    MsgBox Err.Description, vbInformation, "ï‚åáçáäié“àÍóóàÍóóï\"
